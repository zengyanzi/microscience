<?php 
/* @var $this WPAL_Admin_Links */
/* @var $list WPAL_Link_List */
?>
...
<div class="wrap">
	<h2>
		<?php _e('Manage Affiliate Links', 'wpal_plugin') ?>
		&nbsp;
		<a href="<?php echo esc_url(add_query_arg(array('page' => 'wpal-admin-add'), admin_url('admin.php'))) ?>" class="add-new"><?php echo esc_html_x('Add New', 'wpal_plugin'); ?></a>
		<?php if ('' != $s): ?>
	...
	$columns = array(
		'id'	=> __('ID', 'wpal_plugin'),
		'name'	=> __('Name', 'wpal_plugin'),
		'slug'	=> __('URL', 'wpal_plugin'),
	);
	?>
	<ul class="subsubsub">
	...
	<?php endif ?>
	<p class="search-box">
		<label for="link-search-input" class="screen-reader-text"><?php _e('Search Links', 'wpal_plugin') ?>:</label>
		<input id="link-search-input" type="text" name="s" value="<?php echo esc_attr($s) ?>" />
		<input type="submit" class="button" value="<?php _e('Search Links', 'wpal_plugin') ?>">
	</p>
	</form>
	...
	<div class="tablenav">
		<div class="alignleft actions">
			<select name="bulk-action">
				<option value="" selected="selected"><?php _e('Bulk Actions', 'wpal_plugin') ?></option>
				<?php if ('trash' != $type): ?>
					<option value="delete"><?php _e('Move to Trash', 'wpal_plugin') ?></option>
				<?php else: ?>
					<option value="restore"><?php _e('Restore', 'wpal_plugin')?></option>
					<option value="delete"><?php _e('Delete Permanently', 'wpal_plugin')?></option>
				<?php endif ?>
			</select>
			<input type="submit" value="<?php esc_attr_e('Apply', 'wpal_plugin') ?>" name="doaction" id="doaction" class="button-secondary action" />
		</div>
	...
	<div class="clear"></div>
	<table class="widefat wpal-admin-links">
		<thead>
		<tr>
		...
		<tbody id="the-wpal-admin-link-list" class="list:wpal-admin-links">
		<?php if ($list->isEmpty()): ?>
			<tr>
				<td colspan="<?php echo count($columns) + 1 ?>"><?php _e('No links found.', 'wpal_plugin') ?></td>
			</tr>
		<?php else: ?>
			<?php $class = ''; ?>
						...
						switch ($column_id):
							...
							case 'slug':
								...
								<td>
									...
									<div class="row-actions">
										<span class="edit"><a target="_blank" href="<?php echo $item->getURl() ?>">Visit Link</a></span>
									</div>
								</td>
								<?php
								break;
							case 'name':
								...
								<td>
									<strong>
										<?php if ('trash' == $type): ?>
											<span class="row-title"><?php echo '' != $item['name'] ? $item['name'] : __('no title', 'wpal_plugin') ?></span>
										...
										<?php else: ?>
											<span class="row-title"><?php _e('no title', 'wpal_plugin') ?></span>
										<?php endif ?>
											
										<?php if ('_temp' == $item['preset']): ?>
											- <span><?php _e('Draft', 'wpal_plugin') ?></span>
										<?php elseif ('' != $item['preset']): ?>
											- <span><?php echo $item['preset'] ?> <?php _e('Preset', 'wpal_plugin') ?></span>
										<?php elseif ('0000-00-00' != $item['expire_on'] and $item['expire_on'] < date('Y-m-d')): ?>
											- <span><?php _e('Expired', 'wpal_plugin') ?></span>
										<?php endif ?>
									</strong>
									<div class="row-actions">
										<?php if ('trash' != $type): ?>
											<span class="edit"><a class="edit" href="<?php echo esc_url(add_query_arg(array('action' => 'edit', 'id' => $item['id']), $this->baseUrl)) ?>"><?php _e('Edit', 'wpal_plugin') ?></a></span> |
											<?php if ('' == $item['preset']): ?>
												<span class="stats"><a class="stats" href="<?php echo esc_url(add_query_arg(array('page' => 'wpal-admin-statistics', 'id' => $item['id']), admin_url('admin.php'))) ?>"><?php _e('View Stats', 'wpal_plugin') ?></a></span> |
											<?php endif ?>
											<span class="delete"><a class="delete" href="<?php echo esc_url(wp_nonce_url(add_query_arg(array('id' => $item['id'], 'action' => 'delete'), $this->baseUrl), 'delete-link')) ?>"><?php _e('Trash', 'wpal_plugin') ?></a></span>
										<?php else: ?>
											<span class="restore"><a class="restore" href="<?php echo esc_url(wp_nonce_url(add_query_arg(array('id' => $item['id'], 'action' => 'restore'), $this->baseUrl), 'restore-link')) ?>"><?php _e('Restore', 'wpal_plugin') ?></a></span> |
											<span class="delete"><a class="delete" href="<?php echo esc_url(wp_nonce_url(add_query_arg(array('id' => $item['id'], 'action' => 'delete'), $this->baseUrl), 'delete-link')) ?>"><?php _e('Delete Permanently', 'wpal_plugin') ?></a></span>
										<?php endif ?>
									</div>
								</td>
								<?php
	...
	<div class="tablenav">
		...
		<div class="alignleft actions">
			...
			<input type="submit" value="<?php esc_attr_e('Apply', 'wpal_plugin') ?>" name="doaction2" id="doaction2" class="button-secondary action" />
		</div>
	</div>
	<div class="clear"></div>
	</form>
