...
function wpal_wp_loaded() {
	// enable sessions
	if ( ! session_id()) session_start();
	
	// detect if cloaked link is requested and execute redirect.php if so
	if (preg_match('%^' . preg_quote(site_url_no_domain(WPAL_Plugin::getInstance()->getOption('url_prefix')), '%') . '/([\w-]+)(/([^/?]+))?/?($|\?)%', $_SERVER['REQUEST_URI'], $mtch) or preg_match('%^' . preg_quote(site_url_no_domain(), '%') . '/?\?(.*?&)?cloaked=([\w-]+)(&|$)%', $_SERVER['REQUEST_URI'], $mtch_alt)) {
		if ($mtch) {
			$slug = $mtch[1];
			$_GET['subid'] = $mtch[3];
		} else {
