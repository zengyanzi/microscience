 ...
 */
class WPAL_Admin_Keywords extends WPAL_Controller_Admin {
	...
	public function edit() {
		...
		$this->data['item'] = $item = new WPAL_Keyword_Record();
		...
		if ($this->input->post('is_submitted')) {
			...
			if (empty($post['keywords'])) {
				$this->errors->add('form-validation', __('At least one keyword must be set', 'wpal_plugin'));
			} else {
				$keywords = array(); $is_word_error = FALSE;
				foreach (preg_split('%,\s*%', $post['keywords']) as $word) {
					$word = trim($word);
					if ('' != $word) {
						$check = new WPAL_Keyword_Record();
						...
						if ( ! $check->getBy($check_by)->isEmpty()) {
							$this->errors->add('form-validation', sprintf(__('Link for `%s` keyword already defined', 'wpal_plugin'), $word));
							$is_word_error = TRUE;
						} else {
							$keywords[] = $word;
						}
			...
			if (empty($post['url'])) {
				$this->errors->add('form-validation', __('Link To URL must be set', 'wpal_plugin'));
			} 

			// else if ( ! preg_match('%^https?://[\w\d:#@\%/;$()\[\]~_?+=\\\\&.-]+$%i', $post['url'])) {
			// 	$this->errors->add('form-validation', sprintf(__('Specified URL `%s` has wrong format', 'wpal_plugin'), $post['url']));
			// }
			if ( ! preg_match('%^\d+$%', $post['replace_limit'])) {
				$this->errors->add('form-validation', __('Replace Limit must be a non-negative number', 'wpal_plugin'));
			}
			if ($pass_post_id and '' == $post['post_id_param']) {
				$this->errors->add('form-validation', __('Query variable name for Post/Page ID must be set', 'wpal_plugin'));
			}
			...
			if ( ! $this->errors->get_error_codes()) { // no validation errors detected
				...
				if ($id) {
					$item->update();
					$msg = __('Auto-linked keyword updated', 'wpal_plugin');
				} else {
					$item->insert();
					$msg = __('Auto-linked keyword added', 'wpal_plugin');
				}
				wp_redirect(add_query_arg('wpal_nt', urlencode($msg), $this->baseUrl));
			}
		}
		...
		switch ($action) {
			case 'delete':
				if ('trash' == $this->input->get('type')) {
					$msg = sprintf(_n('%d keyword deleted permanentry', '%d keywords deleted permanently', $list->count(), 'wpal_plugin'), $list->count());
				} else {
					$msg = sprintf(_n('%d keyword moved to the Trash', '%d keywords moved to the Trash', $list->count(), 'wpal_plugin'), $list->count());
				}
				break;
			case 'restore':
				$msg = sprintf(_n('%d keyword restored from the Trash', '%d keywords restored from the Trash', $list->count(), 'wpal_plugin'), $list->count());
				break;
		}
	...
	public function restore() {
		...
		$item = new WPAL_Keyword_Record();
		if ( ! $id or $item->getById($id)->isEmpty()) {
			wp_redirect($this->baseUrl);
			die();
		}
		$item->set('is_trashed', 0)->update();
		wp_redirect(add_query_arg('wpal_nt', urlencode(__('Keyword restored from the Trash', 'wpal_plugin')), $this->baseUrl));
		die();
	}
	...
	public function delete() {
		...
		if ($item->is_trashed) {
			$item->delete();
			wp_redirect(add_query_arg('wpal_nt', urlencode(__('Keyword deleted', 'wpal_plugin')), $this->baseUrl));
		} else {
			$item->set('is_trashed', 1)->update();
			wp_redirect(add_query_arg('wpal_nt', urlencode(__('Keyword moved to the Trash', 'wpal_plugin')), $this->baseUrl));
		}
		die();
	}
}