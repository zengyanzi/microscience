 ...
 */
class WPAL_Admin_Statistics extends WPAL_Controller_Admin {
	...
	public function index() {
		...
		$list = new WPAL_Link_List(); $linkTable = $list->getTable();
		$stat = new WPAL_Stat_Record(); $statTable = $stat->getTable();
	...
	public function stats() {
		...
		$this->data['link'] = $link = new WPAL_Link_Record();
		if ($link->getById($id)->isEmpty() or '' != $link->preset or $link->is_trashed) {
			wp_redirect(add_query_arg(array('page' => 'wpal-admin-statistics'), admin_url('admin.php')));
			die();
		}
	...
	public function export()
	{
		if ( ! ($csv = fopen('php://output', 'a'))) {
			wp_die(__('Unable to open stream for outputting CSV file.', 'wpal_plugin'));
		}
		$get = $this->input->get(array(
			'order_by' => 'registered_on',
			'order' => 'DESC',
		...
		$columns = array(
			__('Link ID', 'wpal_plugin'),
			__('Link Name', 'wpal_plugin'),
			__('Click ID', 'wpal_plugin'),
			__('Date/Time', 'wpal_plugin'),
			__('Sub ID', 'wpal_plugin'),
			__('Country Code', 'wpal_plugin'),
			__('IP Address', 'wpal_plugin'),
			__('User Agent', 'wpal_plugin'),
			__('Destination URL', 'wpal_plugin'),
			__('Referrer', 'wpal_plugin'),
		);
		$list = new WPAL_Link_List(); $linkTable = $list->getTable();
		$stat = new WPAL_Stat_Record(); $statTable = $stat->getTable();
		$list->join($statTable, "$statTable.link_id = $linkTable.id")
