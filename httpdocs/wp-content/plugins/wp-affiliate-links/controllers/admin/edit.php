 ...
 */
class WPAL_Admin_Edit extends WPAL_Controller_Admin {
	...
	public function index() {
		...
		$this->data['item'] = $item = new WPAL_Link_Record();
		if ($id and $item->getById($id)->isEmpty()) { // ID corresponds to no link
			wp_redirect(add_query_arg('page', 'wpal-admin-links', admin_url('admin.php')));
			die();
		}
		...
		$rules_by_country = new WPAL_Rule_List();
		$rules_by_rule = new WPAL_Rule_List();
		if ($id) {
			foreach (array('ONE_SET', 'EXPIRED', 'REFERER_MASK') as $type) {
				$default_rules['destination_url'][$type] = array($item->getRule($type)->getRelated('WPAL_Destination_Record')->getUrl());
			}
			...
			foreach ($rules_by_country as $r) {
				...
				$default_rules['destination_url']['BY_COUNTRY'][] = $r->getRelated('WPAL_Destination_Record')->getUrl();
			}
			...
			foreach ($rules_by_rule as $r) {
				...
				$default_rules['destination_url']['BY_RULE'][] = $r->getRelated('WPAL_Destination_Record')->getUrl();
			}
		}
		$rules = $this->input->post($default_rules); $this->data['rules'] =& $rules;
		if ('advanced' == WPAL_Plugin::getInstance()->getOption('destination_mode')) {
		...
		if ($id) {
			$automatch_list = $item->getRelated('WPAL_Automatch_List');
			$automatch_urls = array();
			foreach ($automatch_list as $am) {
				$automatch_urls[] = $am->url;
			}
		} else {
			$automatch_list = new WPAL_Automatch_List();
			$automatch_urls = array();
		}
		...
		if ($this->input->post('is_submitted')) {
			...
			if ('' == $preset) { // preset links can have empty names
				if (empty($post['name'])) {
					$this->errors->add('form-validation', __('Link Name is empty', 'wpal_plugin'));
					unset($item->name);
				} else {
					$check = new WPAL_Link_Record();
					if ( ! $check->getBy(array('name LIKE' => $post['name'], 'preset' => '') + ( ! is_null($id) ? array('id <>' => $id) : array()))->isEmpty()) {
						$this->errors->add('form-validation', sprintf(__('Link with `%s` name already exists (may exist in Trash)', 'wpal_plugin'), $post['name']));
						unset($item->name);
					}
				}
			}
			if ( ! empty($post['slug'])) {
				...
				if ( ! $check->getBy(array('slug LIKE' => $post['slug'], 'preset' => '') + ( ! is_null($id) ? array('id <>' => $id) : array()))->isEmpty()) {
					$this->errors->add('form-validation', sprintf(__('Link with `%s` cloaked URL part already exists (may exist in Trash)', 'wpal_plugin'), $post['slug']));
					unset($item->slug);
				}
			}
			if ( ! empty($post['expire_on'])) {
				if ( ! preg_match('%^\d{4}-\d{2}-\d{2}$%', $post['expire_on'])) {
					$this->errors->add('form-validation', __('Wrong format of expiration date', 'wpal_plugin'));
				...
				foreach ($automatches as $auto_url) {
					...
					if ('' != $auto_url) {
						if ( ! preg_match('%^https?://[\w\d:#@\%/;$()\[\]~_?+=\\\\&.-]+$%i', $auto_url)) {
							$this->errors->add('form-validation', sprintf(__('Auto-Match URL `%s` has wrong format', 'wpal_plugin'), $auto_url));
							$is_automatch_error = TRUE;
						} else {
							if ('' == $preset) {
								$check_links = new WPAL_Automatch_List();
								...
								if ($check_links->total() > 0) {
									$this->errors->add('form-validation', sprintf(__('`%s` URL is already configured to be auto-matched by `%s` link', 'wpal_plugin'), $auto_url, $check_links[0]['name']));
									$is_automatch_error = TRUE;
								} else {
			...
			if ('advanced' == WPAL_Plugin::getInstance()->getOption('destination_mode')) {
				...
				if ('ONE_SET' == $post['destination_type']) {
					if ($item->getRule('ONE_SET')->isEmpty()) {
						$this->errors->add('form-validation', __('Destination for all visitors must be specified', 'wpal_plugin'));
					}
				}
				if ('BY_COUNTRY' == $post['destination_type']) {
					...
					if (array_keys(array_filter($rules['country'])) != array_keys(array_filter($rules['destination_by_country']))) {
						$this->errors->add('form-validation', __('Both country and destination must be sepcified for all rules', 'wpal_plugin'));
					}
					if (array_unique(array_filter($rules['country'])) != array_filter($rules['country'])) {
						$this->errors->add('form-validation', __('Each rule must have unique country specified', 'wpal_plugin'));
					}
				}
				if ('BY_RULE' == $post['destination_type']) {
					...
					if (array_keys(array_filter($rules['rule_name'])) != array_keys(array_filter($rules['rule_pattern'])) or array_keys(array_filter($rules['rule_name'])) != array_keys(array_filter($rules['destination_by_rule']))) {
						$this->errors->add('form-validation', __('Rule, pattern and destination must be sepcified for all rules', 'wpal_plugin'));
					}
					...
					if ($unique_keys != array_keys(array_filter($rules['rule_name'])) and $unique_keys == array_keys(array_unique(array_filter($rules['rule_pattern'])))) {
						$this->errors->add('form-validation', __('Each rule must be unique', 'wpal_plugin'));
					}
					foreach (array_filter($rules['rule_pattern']) as $i => $pattern) {
						if ('REMOTE_ADDR' == $rules['rule_name'][$i]) {
							if ( ! preg_match('%^(\d{1,3}(\.\d{1,3}){3})( *- *(\d{1,3}(\.\d{1,3}){3}))?$%', $pattern, $mtch) or isset($mtch[4]) and ipcmp($mtch[1], $mtch[4]) > 0) {
								$this->errors->add('form-validation', sprintf(__('Pattern `%s` is invalid IP range', 'wpal_plugin'), $pattern));
							}
						} else if ('*' != $rules['rule_name'][$i]) {
							if (FALSE === @preg_match('%' . addcslashes($pattern, '%') . '%i', '')) {
								$this->errors->add('form-validation', sprintf(__('Pattern `%s` is invalid', 'wpal_plugin'), $pattern));
							}
						}
					} 
				}
				...
				foreach (array('ONE_SET', 'EXPIRED', 'REFERER_MASK') as $type) { // ONE_SET, EXPIRED, REFERER_MASK
					...
					if ('' != $url and ! preg_match('%^https?://[\w\d:#@\%/;$()\[\]~_?+=\\\\&.-]+$%i', $url)) {
						$this->errors->add('form-validation', sprintf(__('Specified URL `%s` has wrong format', 'wpal_plugin'), $url));
						$url = ''; // reset wrong url
					}
					if ('' != $url) {
						$rule->set(array(
						...
						$destination = new WPAL_Destination_Record();
			...
			if ($load_preset) {
				...
				$preset_link = new WPAL_Link_Record();
				...
				if ($this->input->post('is_save_as_preset') and '' == $preset) {
					...
					$this->errors->add('form-validation', __('Preset name must be specified', 'wpal_plugin'));
				}
				if ('' != $preset) {
					$check = new WPAL_Link_Record();
					if ($check->getBy(array('preset' => $preset, 'id !=' => $item->id))->isEmpty()) {
						$item->set('preset', $preset)->update();
						wp_redirect(add_query_arg(array('page' => 'wpal-admin-links', 'wpal_nt' => urlencode(__('Link saved as preset', 'wpal_plugin'))) + array_intersect_key($_GET, array_flip($this->baseUrlParamNames)), admin_url('admin.php')));
						die();
					} else {
						$this->errors->errors['form-validation'] = array(); // clear errors
						$this->errors->add('form-validation', __('Preset with specified name already exists', 'wpal_plugin'));
					...
					if ($this->input->get('id')) {
						$msg = __('Link updated', 'wpal_plugin');
					} else {
						$msg = __('Link added', 'wpal_plugin');
					}
					if ('' == $item->preset) {
						$msg .= '. ' . sprintf(__('Cloaked URL is %s', 'wpal_plugin'), '<a href="' . $item->getUrl() . '">' . $item->getUrl() . '</a>');
					}
					wp_redirect(add_query_arg(array('page' => 'wpal-admin-links', 'wpal_nt' => urlencode($msg)) + array_intersect_key($_GET, array_flip($this->baseUrlParamNames)), admin_url('admin.php')));
					die();
	...
	public function destination() {
		...
		$link = new WPAL_Link_Record();
		if ($link_id and $link->getById($link_id)->isEmpty()) { // ID corresponds to no link
			wp_redirect_or_javascript(add_query_arg(array('page' => 'wpal-admin-links'), admin_url('admin.php')));
			die();
		}
		$id = $this->input->getpost('id'); $type = $this->input->getpost('type', 'ONE_SET');
		$this->data['rule'] = $rule = new WPAL_Rule_Record();
		if ($id and ($rule->getById($id)->isEmpty() or $rule->link_id != $link_id or $rule->type != $type)) { // ID corresponds to no destination or rule id corresponds to wrong rule
			wp_redirect_or_javascript(add_query_arg(array('page' => 'wpal-admin-links', 'action' => 'edit', 'link_id' => $link_id), admin_url('admin.php')), 'jQuery("#__modal").dialog("close");');
			die();
		}
		if ( ! $rule->isEmpty()) {
			$default = array();
			foreach ($rule->getRelated('WPAL_Destination_List') as $destination) {
		...
		if ($this->input->post('is_submitted')) {
			...
			if ($this->input->getpost('clear')) { // remove destination at all
				...
				wp_redirect_or_javascript(add_query_arg(array('page' => 'wpal-admin-links', 'action' => 'edit', 'link_id' => $link_id), admin_url('admin.php')), '
					(function ($) {
			...
			if (empty($post['url'])) {
				$this->errors->add('form-validation', __('At least one destination must be set', 'wpal_plugin'));
			} else {
				$weight_total = 0;
				foreach ($post['url'] as $i => $destination) {
					$weight_total += $post['weight'][$i];
					if ( ! preg_match('%^https?://[\w\d:#@\%/;$()\[\]~_?+=\\\\&.-]+$%i', $destination)) {
						$this->errors->add('form-validation', sprintf(__('Specified URL `%s` has wrong format', 'wpal_plugin'), $destination));
					}
				}
				if (100 != $weight_total) {
					$this->errors->add('form-validation', __('Specified weights for links must add up to 100%', 'wpal_plugin'));
				}
			}
	...
	public function destination_upload() {
		if (empty($_FILES['upload']) or empty($_FILES['upload']['name'])) {
			$this->errors->add('form-validation', __('File must be selected', 'wpal_plugin'));
		} else {
			$urls = array_values(array_filter(preg_split('%\s*[,\n\r]\s*%', file_get_contents($_FILES['upload']['tmp_name']))));
			if (empty($urls)) {
				$this->errors->add('form-validation', __('No URLs found in the file uploaded', 'wpal_plugin'));
			}
		}
