...
class WPAL_Destination_Record extends WPAL_Model_Record {
	/**
	...
	public function redirect($type) {
		...
		$rule = $this->getRelated('WPAL_Rule_Record', array('id' => 'rule_id'));
		$link = $rule->getRelated('WPAL_Link_Record', array('id' => 'link_id'));
		if ('REFERER_MASK' == $type) {
			...
			if ( ! $__a) {
				...
				isset($_SERVER['HTTP_REFERER']) and $_SESSION[WPAL_Plugin::PREFIX . 'referer'] = $_SERVER['HTTP_REFERER']
					or $_SESSION[WPAL_Plugin::PREFIX . 'referer'] = '';
			}
			if (empty($_SERVER['HTTP_REFERER']) and ! in_array($__a, array(3, 103))) { // no referer detected (skip 3 and 103 codes since they are intermediate steps of loosing referer with frameset method)
				if (100 <= $__a or '' == trim($link->getTrackingCode('header')) and '' == trim($link->getTrackingCode('footer'))) { // no tracking code required
					...
					echo  '<html><head><title>' . $link->name . '</title><meta http-equiv="refresh" content="' . WPAL_Plugin::getInstance()->getOption('meta_redirect_delay') . '; URL=' . $url . '" />' . $link->getTrackingCode('header') . '</head><body>' . $link->getTrackingCode('footer') . '</body></html>';
				}
			...
			if (WPAL_Plugin::getInstance()->getOption('forward_url_params') and $link->forward_url_params) { // forward query params
			...
			$geoip = new WPAL_GeoIPCountry_Record();
			
			// detect referrer
			$referer = '';
			if ('REFERER_MASK' == $link->redirect_type) {
				isset($_SESSION[WPAL_Plugin::PREFIX . 'referer']) and $referer = $_SESSION[WPAL_Plugin::PREFIX . 'referer'];
			} else {
				isset($_SERVER['HTTP_REFERER']) and $referer = $_SERVER['HTTP_REFERER'];
			}
			unset($_SESSION[WPAL_Plugin::PREFIX . 'referer']);
			
			// create stat object
			$stat = new WPAL_Stat_Record(array(
				'link_id' => $link->id,
