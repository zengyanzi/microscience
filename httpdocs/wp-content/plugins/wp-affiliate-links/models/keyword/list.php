...
class WPAL_Keyword_List extends WPAL_Model_List {
					...
					foreach ($chanks_tags as $i => $c_tags) {
						if ($i % 2 == 0 and '' != $c_tags) { // skip boundaries and empty strings
							foreach ($this as $keyword) {
								$a_tag_template = '<a class="wpal-linked-keyword" href="' . ($post_id && '' != $keyword['post_id_param'] ? add_query_arg($keyword['post_id_param'], $post_id, $keyword['url']) : $keyword['url']) . '"' . ($keyword['rel_nofollow'] ? ' rel="nofollow"' : '') . ($keyword['target_blank'] ? ' target="_blank"' : '') . '>$0</a>';
