 ...
 */
class WPAL_Model_Record extends WPAL_Model {
	/**
	 * Initialize model
	...
	/**
	 * @see WPAL_Model::getBy()
	 * @return WPAL_Model_Record
	 */
	 ...
	 * @return WPAL_Model_List
	 */
	public function getRelated($model, $keyAssoc = NULL) {
		$related = new $model();
		if ( ! empty($this->id)) {
			if (is_null($keyAssoc)) {
				$defaultPrefix = strtolower(preg_replace('%^' . strtoupper(WPAL_Plugin::PREFIX) . '|_Record$%', '', get_class($this)));
				$keyAssoc = array();
		...
		return $related instanceof WPAL_Model_List ? $related->convertRecords() : $related;
	}
