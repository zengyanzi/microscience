<?php
/*
Plugin Name: WP Affiliate Links
Plugin URI: http://www.dojo.cc
Description: Cloak, track, redirect your links, split test offers by country, hide your referrers from merchants and affiliate networks!
Version: 0.1.1
 ...
 * @var string
 */
define('WPAL_ROOT_DIR', str_replace('\\', '/', dirname(__FILE__)));
/**
 * Plugin root url for referencing static content
 * @var string
 */
define('WPAL_ROOT_URL', rtrim(plugin_dir_url(__FILE__), '/'));
/**
...
final class WPAL_Plugin {
	 ...
	 * @var string
	 */
	const ROOT_DIR = WPAL_ROOT_DIR;
	/**
	 * Plugin root URL
	 * @var string
	 */
	const ROOT_URL = WPAL_ROOT_URL;
	/**
	 * Prefix used for names of shortcodes, action handlers, filter functions etc.
	 * @var string
	 */
	const PREFIX = WPAL_PREFIX;
	/**
	 * Plugin file path
	 * @var string
	 */
	...
	public function __infoApiAuth($args, $url) {
		if (dirname($url) == dirname(WPAL_Plugin::getInstance()->getOption('info_api_url'))) {
	...
	public function shortcodeDispatcher($args, $content, $tag) {
		...
		if ( ! $controller instanceof WPAL_Controller) {
	...
	public function adminDispatcher($page = '', $action = 'index') {
		...
		if ('' === $page) {
			...
			if ( ! $this->isLicensed() and ! in_array($controllerName, array('WPAL_Admin_Help', 'WPAL_Admin_Home'))) {
				wp_redirect(add_query_arg('page', 'wpal-admin-home', admin_url('admin.php')));
			}
			...
			if (method_exists($controllerName, $actionName)) {
				...
				if ( ! get_current_user_id() or ! current_user_can('manage_options')) {
					...
					$controller = new $controllerName();
					if ( ! $controller instanceof WPAL_Controller_Admin) {
	...
	public function __activation() {
		...
		$options_default = WPAL_Config::createFromFile(self::ROOT_DIR . '/config/options.php')->toArray();
		...
		$csv = NULL;
		is_file(WPAL_Plugin::ROOT_DIR . '/GeoIPCountryWhois.csv') and $csv = fopen(WPAL_Plugin::ROOT_DIR . '/GeoIPCountryWhois.csv', 'r'); // try raw file
		...
		if (empty($csv) and function_exists('gzopen')) { // try gz
			 is_file(WPAL_Plugin::ROOT_DIR . '/GeoIPCountryWhois.csv.gz') and ($csv = fopen('compress.zlib://' . WPAL_Plugin::ROOT_DIR . '/GeoIPCountryWhois.csv.gz', 'r'));
		}

		if ($csv) {
			global $wpdb;
			$record = new WPAL_GeoIPCountry_Record();
...
WPAL_Plugin::getInstance()->recurringLicenseCheck();
